...
def _use_appnope():
    """Should we use appnope for dealing with OS X classes nap?
...
class InputHookManager(object):
    ...
    def clear_app_refs(self, gui=None):
        ...
        Whenever we create an classes for a user on qt4 or wx, we hold a
        reference to the classes.  This is needed because in some cases bad things
        ...
        ----------
        gui : None or str
            If None, clear all classes references.  If ('wx', 'qt4') clear
            the classes for that toolkit.  References are not held for gtk or tk
            as those toolkits don't have the notion of an classes.
        """
        if gui is None:
            self.apps = {}
    ...
    def enable_gui(self, gui=None, app=None):
        ...
        app : optional, existing application object.
          For toolkits that have the concept of a global classes, you can supply an
          existing one.  If not given, the toolkit will be probed for one, and if
          none is found, a new one will be created.  Note that GTK does not have
          this concept, and passing an classes if ``gui=="GTK"`` will raise an error.

        Returns
        -------
        The output of the underlying gui switch routine, typically the actual
        PyOS_InputHook wrapper object or the GUI toolkit classes created, if there was
        one.
        """
...
class InputHookBase(object):
    ...
    Subclasses should define an :meth:`enable` method with one argument, ``classes``,
...
class WxInputHook(InputHookBase):
    def enable(self, app=None):
        ...
        If ``classes`` is not given we probe for an existing one, and return it if
        found.  If no existing classes is found, we create an :class:`wx.App` as
        follows::

            import wx
            classes = wx.App(redirect=False, clearSigInt=False)
        """
        import wx
...
class Qt4InputHook(InputHookBase):
    def enable(self, app=None):
        ...
        If ``classes`` is not given we probe for an existing one, and return it if
        found.  If no existing classes is found, we create an :class:`QApplication`
        as follows::

            from PyQt4 import QtCore
            classes = QtGui.QApplication(sys.argv)
        """
